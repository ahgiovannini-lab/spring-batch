...
## Decisão
Implementar um `ItemReader` customizado (`ChunkCheckpointingReader`) que é `ItemStream` e mantém um `committedOffset` 
(posição da próxima linha após o último chunk confirmado). O offset é o único valor persistido no `ExecutionContext` no momento do commit do chunk. 
